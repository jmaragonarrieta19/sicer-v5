...
# ==========================================
st.set_page_config(page_title="SICER IA", layout="wide", initial_sidebar_state="collapsed")
...
for key in ["usuarios", "empresas", "propiedades_dinamicas", "lista_carac", "mensaje_intro", "fuente_intro", "tamano_intro", "align_intro", "linea_intro", "caracteristicas_18", "mensaje_final", "fuente_final", "tamano_final", "align_final", "linea_final", "solicitudes", "api_key", "model_name", "titulo_adicional", "texto_adicional", "firmas_config"]:
    if key not in st.session_state: st.session_state[key] = st.session_state.config_db.get(key, "")
...
# ==========================================
# 8. FUNCIÓN PDF SEGURA (SOPORTA PLANTILLA Y MERGE)
# ==========================================
...
def generar_pdf(empresa_emisora, ruc_emisor, cliente, ruc, comprobante, vendedor, ciudad, props_dinamicas, cantidad, carac_18, msg_final, num_cert, incluir_proteccion, texto_proteccion):
    ...
    if not usar_plantilla:
        ...
        posiciones = [(20, 60), (120, 60), (20, 140), (120, 140), (20, 220), (120, 220)]
        for x_pos, y_pos in posiciones:
            pdf.rotate(30, x_pos, y_pos)
    ...
    try:
        img_qr.save(qr_path, format="JPEG")
...
def tab_emision():
                                ...
                                if "CLIENTE" in datos or "EMISOR" in datos:
                                    ...
                                    if not extracted_ruc_clean or extracted_ruc_clean not in allowed_rucs_clean:
                                        ...
                                        st.session_state.datos_form.update({
                                            ...
                                            "comprobante": comprobante_final, "vendedor": vendedor_final, 
                                            "ciudad": ciudad_final, "cantidad": datos.get("CANTIDAD", "")
                                        })
                                        
                                        uk = st.session_state.uploader_key
                                        st.session_state[f"e_{uk}"] = datos.get("EMISOR", "")
                                        st.session_state[f"re_{uk}"] = datos.get("RUC_EMISOR", "")
                                        st.session_state[f"c_{uk}"] = datos.get("CLIENTE", "")
                                        st.session_state[f"r_{uk}"] = datos.get("RUC_DNI_CLIENTE", "")
                                        st.session_state[f"co_{uk}"] = comprobante_final
                                        st.session_state[f"v_{uk}"] = vendedor_final
                                        st.session_state[f"ci_{uk}"] = ciudad_final
                                        st.session_state[f"ca_{uk}"] = datos.get("CANTIDAD", "")
    ...
    with st.container(border=True):
        ...
        ruc_actual = st.session_state.datos_form.get("ruc_emisor", "")
        ruc_actual_clean = re.sub(r'[^0-9]', '', ruc_actual)
        ...
        if ruc_actual_clean:
            for emp in st.session_state.empresas:
                if re.sub(r'[^0-9]', '', str(emp.get("ruc", ""))) == ruc_actual_clean:
                    ...
                    break
        st.session_state.datos_form["ciudad"] = ciudad_dinamica
        
        c_r1_1, c_r1_2 = st.columns([6, 4], gap="medium")
        st.session_state.datos_form["empresa"] = c_r1_1.text_input("Empresa Emisora", value=st.session_state.datos_form.get("empresa", ""), disabled=bloquear_campos, key=f"e_{st.session_state.uploader_key}")
        st.session_state.datos_form["ruc_emisor"] = c_r1_2.text_input("RUC Emisor", value=st.session_state.datos_form.get("ruc_emisor", ""), disabled=bloquear_campos, key=f"re_{st.session_state.uploader_key}")

        c_r2_1, c_r2_2, c_r2_3, c_r2_4, c_r2_5 = st.columns([3, 2, 2, 2, 2], gap="medium")
        st.session_state.datos_form["cliente"] = c_r2_1.text_input("Cliente", value=st.session_state.datos_form.get("cliente", ""), disabled=bloquear_campos, key=f"c_{st.session_state.uploader_key}")
        st.session_state.datos_form["ruc"] = c_r2_2.text_input("RUC / DNI", value=st.session_state.datos_form.get("ruc", ""), disabled=bloquear_campos, key=f"r_{st.session_state.uploader_key}") 
        st.session_state.datos_form["comprobante"] = c_r2_3.text_input("N° Comprobante", value=st.session_state.datos_form.get("comprobante", ""), placeholder="Ej. F001-000001", disabled=bloquear_campos, key=f"co_{st.session_state.uploader_key}")
        st.session_state.datos_form["vendedor"] = c_r2_4.text_input("Vendedor", value=st.session_state.datos_form.get("vendedor", ""), disabled=bloquear_campos, key=f"v_{st.session_state.uploader_key}")
        st.session_state.datos_form["ciudad"] = c_r2_5.text_input("Ciudad Emisión", value=ciudad_dinamica, disabled=bloquear_campos, key=f"ci_{st.session_state.uploader_key}")
        
        st.session_state.datos_form["cantidad"] = st.text_area("Detalle de Productos (Soporta HTML: <b>, <i>, <br>)", value=st.session_state.datos_form.get("cantidad", ""), height=150, key=f"ca_{st.session_state.uploader_key}")
        
        st.write("")
        st.session_state.datos_form["incluir_proteccion"] = st.checkbox("Incluir cláusula de Protección Adicional en este certificado", value=st.session_state.datos_form.get("incluir_proteccion", True), key=f"prot_check_{st.session_state.uploader_key}")
...
def tab_identidad():
    section_header(ICON_BUILDING, "Datos de la Empresa y Catálogo")
    with st.container(border=True):
        ...
        with st.form("form_add_emp", clear_on_submit=True):
            c_cat1, c_cat2, c_cat3 = st.columns([3, 2, 3], gap="medium")
    ...
    with st.container(border=True):
        ...
        for emp in list(st.session_state.empresas):
            col_e1, col_e2, col_e3, col_e4 = st.columns([3, 2, 3, 1])
            ...
            if len(st.session_state.empresas) > 1:
                if col_e4.button("Eliminar", key=f"del_emp_{emp['nombre']}", type="secondary", use_container_width=True):
    ...
    with st.container(border=True):
        ...
        if emp_visual:
            st.write("")
            c_img1, c_img2, c_img3 = st.columns([1, 1, 1], gap="large")
            with c_img1:
                ...
                if os.path.exists(ruta_l):
                    ...
                    if st.button("Eliminar Logo", key="dl_btn", type="secondary", use_container_width=True): os.remove(ruta_l); st.rerun()
                else:
            ...
            with c_img3:
                ...
                for f_data in firmas_emp:
                    col_f1, col_f2, col_f3 = st.columns([2, 3, 2], vertical_alignment="center")
                    f_path = os.path.join(os.getcwd(), f"firma_{limpiar_nombre(emp_visual)}_{f_data['id']}.png")
                    if os.path.exists(f_path):
                        col_f1.markdown(obtener_imagen_html(f_path, altura_px=40), unsafe_allow_html=True)
                    col_f2.markdown(f"<span style='color: #1E293B; font-size:0.9rem;'><b>{f_data['cargo']}</b></span>", unsafe_allow_html=True)
                    if col_f3.button("Quitar", key=f"del_f_{f_data['id']}", type="secondary"):
                ...
                with st.form(f"form_add_firma_{emp_visual}", clear_on_submit=True):
                    new_f_file = st.file_uploader("Subir firma (.png, .jpg)", type=["png", "jpg"])
                    new_f_cargo = st.text_input("Cargo (Ej. GERENTE GENERAL)")
                    if st.form_submit_button("Añadir Firma", type="primary"):
    ...
    with st.container(border=True):
        ...
        for idx_dyn, prop_dyn in enumerate(st.session_state.propiedades_dinamicas):
            ...
            if col_d3.button("Eliminar", key=f"del_dyn_{uid}", type="secondary"):
        ...
        with c_us2:
            ...
            for u, dat in list(st.session_state.usuarios.items()):
                ...
                if u != st.session_state.usuario_actual:
                    if col_row5.button("Eliminar", key=f"btn_del_{u}", type="secondary"):
...
def tab_historial_general():
    ...
    def puede_ver_solicitud(sol):
        ...
        return any(emp in emisor_empresas for emp in vendor_empresas)

    if st.session_state.rol == "administrador":
        historial_filtrado = st.session_state.historial_db
    else:
        historial_filtrado = [c for c in st.session_state.historial_db if c.get("Emisor") in emisor_empresas]
    ...
    with st.container(border=True):
        if len(historial_filtrado) > 0:
            ...
            if is_admin:
                ...
                c6.markdown("<span style='color:#94A3B8; font-weight:600; font-size:0.8rem; letter-spacing:0.5px;'>ACCIÓN</span>", unsafe_allow_html=True)
            else:
            ...
            c4.markdown("<span style='color:#94A3B8; font-weight:600; font-size:0.8rem; letter-spacing:0.5px;'>VENDEDOR</span>", unsafe_allow_html=True)
            c5.markdown("<span style='color:#94A3B8; font-weight:600; font-size:0.8rem; letter-spacing:0.5px;'>ARCHIVO</span>", unsafe_allow_html=True)
            ...
            for cert in reversed(historial_filtrado):
                if is_admin:
                    c1, c2, c3, c4, c5, c6 = st.columns([2, 2, 2, 2, 2, 2], vertical_alignment="center")
                else:
                    c1, c2, c3, c4, c5 = st.columns([2, 2, 3, 2, 2], vertical_alignment="center")
                    
                c1.write(cert["Fecha"])
                c2.write(cert["Comprobante"])
                c3.write(cert["Cliente"])
                c4.write(cert.get("Vendedor", ""))
                ...
                if ruta and os.path.exists(ruta):
                    ...
                    c5.markdown("<span style='color:#94A3B8;'>No disp.</span>", unsafe_allow_html=True)
                    
                if is_admin:
                    if c6.button("Eliminar", key=f"del_cert_{cert['N_Cert']}", type="secondary", use_container_width=True):
    ...
    with st.container(border=True):
        if len(rechazadas_filtradas) > 0:
            ...
            if is_admin:
                ...
                c5.markdown("<span style='color:#94A3B8; font-weight:600; font-size:0.8rem; letter-spacing:0.5px;'>ACCIÓN</span>", unsafe_allow_html=True)
            else:
            ...
            for s in reversed(rechazadas_filtradas):
                if is_admin:
                    c1, c2, c3, c4, c5 = st.columns([2, 2, 2, 4, 2], vertical_alignment="center")
                else:
                ...
                if is_admin:
                    if c5.button("Eliminar", key=f"del_rech_gen_{s['id']}", type="secondary", use_container_width=True):
    ...
    with st.container(border=True):
        ...
        if verificar_btn:
            if not comp_verif.strip() or not doc_verif.strip():
                ...
                def limpiar_cadena(s): return re.sub(r'[^a-zA-Z0-9]', '', str(s)).upper()
                
                doc_in = re.sub(r'[^0-9]', '', str(doc_verif))
                comp_in = limpiar_cadena(comp_verif)
                emp_in = limpiar_cadena(empresa_verif)
                
                encontrado = None
                for cert in st.session_state.historial_db:
                    doc_db = re.sub(r'[^0-9]', '', str(cert.get("Documento", "")))
                    comp_db = limpiar_cadena(cert.get("Comprobante", ""))
                    emp_db = limpiar_cadena(cert.get("Emisor", ""))
...
if not st.session_state.logueado:
    st.markdown("""
        ...
        <div class="navbar-logo">SICER <span>IA v5.1</span></div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.rol == "administrador":
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["Emisión", "Datos de la Empresa y Catálogo", "Usuarios", "Historial", "Motor IA"])
        with tab1: tab_emision()
        with tab2: tab_identidad()
        with tab3: tab_usuarios()
        with tab4: tab_historial_general()
