...
# ==========================================
# 8. FUNCIÓN PDF SEGURA (SIN SALTOS DE PÁGINA)
# ==========================================
...
def generar_pdf(empresa_emisora, ruc_emisor, cliente, ruc, comprobante, vendedor, ciudad, props_dinamicas, cantidad, carac_18, msg_final, num_cert, incluir_proteccion, texto_proteccion):
    ...
    # REPARACIÓN CRÍTICA: Restaurar coordenadas a la parte superior.
    # Al insertar una imagen completa, FPDF mueve el cursor al fondo (y=297).
    pdf.set_xy(10, 15)
    ...
    # Restaurar de nuevo por seguridad
    pdf.set_xy(10, 15)
...
def tab_configuracion_diseno():
    ...
    with st.container(border=True):
        ...
        if emp_visual:
            ...
            with c_img2:
                ...
                if os.path.exists(ruta_p):
                    ...
                    if file_p and st.button("Guardar Plantilla", key="sp_p_btn", type="primary", use_container_width=True):
                        ...
                        img.save(obtener_ruta_plantilla(emp_visual), "JPEG", quality=85)
                        st.rerun()
            ...
            st.write("")
            if firmas_emp:
                cols_firmas = st.columns(3, gap="medium")
                for idx, f_data in enumerate(firmas_emp):
                    with cols_firmas[idx % 3].container(border=True):
                        ...
                        if st.button("Eliminar", key=f"del_f_{f_data['id']}", type="secondary", use_container_width=True):
