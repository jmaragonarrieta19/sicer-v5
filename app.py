...
# ==========================================
st.set_page_config(page_title="SICER IA v8.0", layout="wide", initial_sidebar_state="collapsed")
...
# ==========================================
# 8. FUNCIÓN PDF SEGURA (SOLUCIÓN DE LENTITUD Y COMPATIBILIDAD)
# ==========================================
...
def generar_pdf(empresa_emisora, ruc_emisor, cliente, ruc, comprobante, vendedor, ciudad, props_dinamicas, cantidad, carac_18, msg_final, num_cert, incluir_proteccion, texto_proteccion):
    ...
    # ---------------------------------------------------------
    # DIBUJAR PLANTILLA COMO FONDO JPG 
    # ---------------------------------------------------------
    ...
    try:
        # Se elimina optimize=True para que FPDF clásico no colapse (JPEGs progresivos son incompatibles)
        ...
        # ---------------------------------------------------------
        # REPARACIÓN DE COMPATIBILIDAD CON FPDF Y FPDF2 (Sin cuelgues)
        # ---------------------------------------------------------
        try:
                            ...
                            try:
                                ...
                                if "CLIENTE" in datos or "EMISOR" in datos:
                                    ...
                                    if not extracted_ruc_clean or extracted_ruc_clean not in allowed_rucs_clean:
                                        ...
                                        # ---------------------------------------------------------
                                        # REPARACIÓN DE ADVERTENCIAS Y LAG EN STREAMLIT
                                        # Ya no se fuerza st.session_state[key], solo el diccionario.
                                        # ---------------------------------------------------------
...
def tab_configuracion_diseno():
    ...
    with st.container(border=True):
        ...
        if emp_visual:
            ...
            with c_img2:
                # ---------------------------------------------------------
                # REPARACIÓN DE PLANTILLAS: OPTIMIZACIÓN EXTREMA SIN PROGRESSIVE JPEG
                # ---------------------------------------------------------
                ...
                if os.path.exists(ruta_p):
                    ...
                    if file_p and st.button("Guardar Plantilla", key="sp_p_btn", type="primary", use_container_width=True):
                        ...
                        img.save(obtener_ruta_plantilla(emp_visual), "JPEG", quality=85) # Explicitly NO optimize
                        st.rerun()
...
if not st.session_state.logueado:
    st.markdown("""
        ...
        <div class="navbar-logo">SICER <span>IA v8.0</span></div>
    """, unsafe_allow_html=True)
            ...
            st.markdown("""
                <div style='text-align: center; margin-bottom: 30px;'>
                    <h1 style='color: #1E293B; font-weight: 900; font-style: italic; font-size: 3.5rem; letter-spacing: -1.5px; margin:0;'>
                        SICER <span style='background: linear-gradient(135deg, #4E008E 0%, #6A0DAD 100%); color: white; padding: 4px 15px; border-radius: 99px; font-size: 1.4rem; font-style: normal; vertical-align: middle; margin-left:10px;'>IA v8.0</span>
                    </h1>
