...
# ==========================================
st.set_page_config(page_title="SICER IA v5.3", layout="wide", initial_sidebar_state="collapsed")
...
def tab_ajustes_sistema():
    ...
    with st.container(border=True):
        st.markdown("<p style='font-size:0.95rem; color:#64748B;'>Configure las credenciales de la API de Google Gemini y el modelo activo. Los cambios se aplign de inmediato en todo el sistema.</p>", unsafe_allow_html=True)
...
def tab_historial_general():
    ...
    # ----------------------------------------------------
    # NORMALIZACIÓN AVANZADA CONTRA PUNTOS, COMAS Y ABREVIATURAS
    # ----------------------------------------------------
    def limpiar_texto_empresa(texto):
        if not texto: return ""
        t = str(texto).upper()
        # Eliminar puntos, comas, guiones y espacios múltiples
        t = re.sub(r'[.,\-_/]', ' ', t)
        # Normalizar abreviaturas comerciales comunes para que "S.A.C." sea igual a "SAC"
    ...
    if st.session_state.rol == "administrador":
        ...
        # Cruce estricto pero tolerante a puntos y comas con la IA
        ...
        if verificar_btn:
            if not comp_verif.strip() or not doc_verif.strip():
                ...
                def limpiar_texto_buscador(s):
                    ...
                    t = re.sub(r'\bS\s*A\b', 'SA', t)
                    t = re.sub(r'\bE\s*I\s*R\s*L\b', 'EIRL', t)
                    return re.sub(r'\s+', '', t) # Eliminar todos los espacios para cruce perfecto
...
if not st.session_state.logueado:
    st.markdown("""
        ...
        <div class="navbar-logo">SICER <span>IA v5.3</span></div>
    """, unsafe_allow_html=True)
            ...
            st.markdown("""
                <div style='text-align: center; margin-bottom: 30px;'>
                    <h1 style='color: #1E293B; font-weight: 900; font-style: italic; font-size: 3.5rem; letter-spacing: -1.5px; margin:0;'>
                        SICER <span style='background: linear-gradient(135deg, #4E008E 0%, #6A0DAD 100%); color: white; padding: 4px 15px; border-radius: 99px; font-size: 1.4rem; font-style: normal; vertical-align: middle; margin-left:10px;'>IA v5.3</span>
                    </h1>
