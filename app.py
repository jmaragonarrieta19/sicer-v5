...
# ==========================================
st.set_page_config(page_title="SICER IA v5.4", layout="wide", initial_sidebar_state="collapsed")
...
# ==========================================
# 8. FUNCIÓN PDF SEGURA (QR ROBUSTO)
# ==========================================
...
def generar_pdf(empresa_emisora, ruc_emisor, cliente, ruc, comprobante, vendedor, ciudad, props_dinamicas, cantidad, carac_18, msg_final, num_cert, incluir_proteccion, texto_proteccion):
    ...
    # NUEVO: Formato optimizado y ultralimpio para lectura perfecta por cualquier cámara de celular
    qr_data = f"CERT: {num_cert} | COMP: {comprobante} | EMP: {empresa_emisora} | RUC: {ruc_emisor}"
...
def tab_historial_general():
    ...
    with st.container(border=True):
        if len(historial_filtrado) > 0:
            ...
            # NUEVO: Columna de EMPRESA añadida y simetría ajustada
            if is_admin:
...
if not st.session_state.logueado:
    st.markdown("""
        ...
        <div class="navbar-logo">SICER <span>IA v5.4</span></div>
    """, unsafe_allow_html=True)
            ...
            st.markdown("""
                <div style='text-align: center; margin-bottom: 30px;'>
                    <h1 style='color: #1E293B; font-weight: 900; font-style: italic; font-size: 3.5rem; letter-spacing: -1.5px; margin:0;'>
                        SICER <span style='background: linear-gradient(135deg, #4E008E 0%, #6A0DAD 100%); color: white; padding: 4px 15px; border-radius: 99px; font-size: 1.4rem; font-style: normal; vertical-align: middle; margin-left:10px;'>IA v5.4</span>
                    </h1>
