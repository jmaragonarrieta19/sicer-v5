...
def section_header(title, subtitle=""):
    st.markdown(f"""
        <div style="margin-top: 10px; margin-bottom: 25px; padding-bottom: 10px; border-bottom: 1px solid #E2E8F0;">
            <h2 style="margin: 0; font-size: 1.5rem; color: #1E293B; font-weight: 800; letter-spacing: -0.5px;">{title}</h2>
...
# ==========================================
# 4. CONFIGURACIÓN DE PÁGINA Y CSS (KASNET STYLE)
# ==========================================
st.set_page_config(page_title="SICER IA v11.0", layout="wide", initial_sidebar_state="expanded")
...
<style>
    ...
    [data-testid="stSidebar"] [data-testid="stRadio"] label:hover { background-color: #F1F5F9 !important; }
    ...
    button[kind="primary"]:hover { background-color: #4338CA !important; box-shadow: 0 4px 6px rgba(79, 70, 229, 0.3) !important; transform: translateY(-1px); }
    ...
    button[kind="secondary"]:hover { background-color: #F8FAFC !important; color: #1E293B !important; border-color: #94A3B8 !important; }
</style>
""", unsafe_allow_html=True)

# ==========================================
# 4B. INYECCIÓN DINÁMICA DE SVG EN MENU LATERAL
# ==========================================
def inject_sidebar_svg_css(menu_items):
    svg_paths = {
        "Inicio": "M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2zM9 22V12h6v10",
        ...
        "Transacciones": "M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10zM12 6v6l4 2",
        "Configuración": "M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1zM12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z",
        "Motor IA": "M12 2a10 10 0 1 0 10 10H12V2zM12 12L2.1 7.9M12 12l9.9-4.1",
        "Solicitar Certificado": "M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M16 13H8M16 17H8M10 9H8",
        "Mis Operaciones": "M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16zM3.27 6.96L12 12.01l8.73-5.05M12 22.08V12"
    }
    
    css = "<style>"
    for i, item in enumerate(menu_items):
        path = svg_paths.get(item, svg_paths["Inicio"])
        svg_code = f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="black" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="{path}"></path></svg>'
        css += f"""
        div[role="radiogroup"] label:nth-child({i+1}) div[dir="auto"]::before {{
            content: ''; display: inline-block; width: 20px; height: 20px;
            background-color: #64748B;
            -webkit-mask-image: url('data:image/svg+xml;utf8,{svg_code}');
            mask-image: url('data:image/svg+xml;utf8,{svg_code}');
            -webkit-mask-size: cover; mask-size: cover;
            margin-right: 14px; vertical-align: bottom; transition: background-color 0.2s;
        }}
        div[role="radiogroup"] label[aria-checked="true"]:nth-child({i+1}) div[dir="auto"]::before {{
            background-color: #4F46E5 !important;
        }}
        """
    css += "</style>"
...
def obtener_imagen_html(ruta_imagen, altura_px=60):
    ...
    return f'<img src="data:image/png;base64,{encoded_string}" style="max-height: {altura_px}px; max-width: 100%; object-fit: contain;">'
...
def tab_configuracion_diseno():
    ...
    with st.container(border=True):
        st.markdown("<p style='font-weight:800; color:#1E293B;'>Directorio</p>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        if emp_visual:
            ...
            with c_img1:
                st.markdown("<p style='font-weight:800; font-size:0.9rem;'>LOGO CORPORATIVO</p>", unsafe_allow_html=True)
                ruta_l = obtener_ruta_logo(emp_visual)
                if os.path.exists(ruta_l):
                    with st.container(border=True):
                        c1, c2, c3 = st.columns([1.5, 3, 1], vertical_alignment="center")
                        c1.markdown(obtener_imagen_html(ruta_l, 45), unsafe_allow_html=True)
                        c2.markdown("**Logo Corporativo**<br><span style='color:#10B981; font-size:0.8rem;'>✅ Activo</span>", unsafe_allow_html=True)
                        if c3.button("🗑️", key="dl_btn", help="Quitar logo", use_container_width=True): os.remove(ruta_l); st.rerun()
                else:
                    file_l = st.file_uploader("Subir logo (.png, .jpg)", type=["png", "jpg"], label_visibility="collapsed")
            ...
            with c_img2:
                st.markdown("<p style='font-weight:800; font-size:0.9rem;'>PLANTILLA (.PDF)</p>", unsafe_allow_html=True)
                ruta_p = obtener_ruta_plantilla(emp_visual)
                if os.path.exists(ruta_p):
                    with st.container(border=True):
                        c1, c2, c3 = st.columns([1.5, 3, 1], vertical_alignment="center")
                        c1.markdown("<h1 style='text-align:center; color:#4F46E5; margin:0;'>📄</h1>", unsafe_allow_html=True)
                        c2.markdown("**Plantilla de Fondo**<br><span style='color:#10B981; font-size:0.8rem;'>✅ Activa (.pdf)</span>", unsafe_allow_html=True)
                        if c3.button("🗑️", key="dp_btn", help="Quitar plantilla", use_container_width=True): os.remove(ruta_p); st.rerun()
                else:
            ...
            st.markdown("<p style='font-weight:800;'>Firmas Autorizadas</p>", unsafe_allow_html=True)
            ...
            if firmas_emp:
                ...
                for idx, f_data in enumerate(firmas_emp):
                    with cols_firmas[idx % 4].container(border=True):
                        ...
                        st.markdown(f"<div style='text-align:center; font-size:0.8rem; font-weight:800; color:#334155; margin-bottom:10px;'>{f_data['cargo'].upper()}</div>", unsafe_allow_html=True)
                        if st.button("🗑️ Eliminar", key=f"del_f_{f_data['id']}", use_container_width=True):
...
def tab_ajustes_sistema():
    section_header("Motor de Inteligencia Artificial")
    with st.container(border=True):
...
def tab_usuarios():
    ...
    with st.container(border=True):
        st.markdown("<p style='font-weight:800; color:#1E293B;'>Nuevo Usuario</p>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        st.markdown("<p style='font-weight:800; color:#1E293B;'>Directorio Activo</p>", unsafe_allow_html=True)
...
def tab_historial_general():
    ...
    with st.container(border=True):
        st.markdown("<p style='font-weight:800; color:#1E293B;'>Certificados Emitidos</p>", unsafe_allow_html=True)
        if len(historial_filtrado) > 0:
            ...
            c6.markdown("**EMISOR**")
            c7.markdown("<div style='text-align:center;'>**DOC.**</div>", unsafe_allow_html=True)
            if is_admin: c8.markdown("<div style='text-align:center;'>**DEL**</div>", unsafe_allow_html=True)
            ...
            for cert in reversed(historial_filtrado):
                ...
                if ruta and os.path.exists(ruta): c7.markdown(f"<div style='text-align:center;'>{get_download_icon(ruta)}</div>", unsafe_allow_html=True)
...
def tab_historial_vendedor():
    ...
    with st.container(border=True):
        ...
        if mis_certificados:
            ...
            c3.markdown("**CLIENTE**")
            c4.markdown("<div style='text-align:center;'>**DOC.**</div>", unsafe_allow_html=True)
            ...
            for cert in reversed(mis_certificados):
                ...
                ruta = cert.get("Ruta_PDF")
                if ruta and os.path.exists(ruta): c4.markdown(f"<div style='text-align:center;'>{get_download_icon(ruta)}</div>", unsafe_allow_html=True)
                st.markdown("<hr style='margin: 5px 0;'>", unsafe_allow_html=True)
...
def vista_verificacion_publica():
    st.markdown("""
        <div style='text-align: center; margin-top: 20px; margin-bottom: 40px;'>
            <div style='display: inline-block; background-color: #EEF2FF; padding: 20px; border-radius: 50%; margin-bottom: 15px;'>
                <svg width="45" height="45" viewBox="0 0 24 24" fill="none" stroke="#4F46E5" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><circle cx="10" cy="13" r="2"></circle><line x1="11.4" y1="14.4" x2="13.5" y2="16.5"></line></svg>
            </div>
...
# ==========================================
# 8. ENRUTADOR Y MENÚ KASNET
# ==========================================
if not st.session_state.logueado:
    st.markdown('<div class="top-navbar-bg"></div><div class="navbar-logo">SICER <span>IA v11.0</span></div>', unsafe_allow_html=True)
    ...
    # TOP NAVBAR TIPO KASNET
    # ---------------------------------------------------------
    rol_text = ROLES_TXT.get(st.session_state.rol, 'Usuario')
    st.markdown(f"""
        <div class="kasnet-topbar">
            <div style="display: flex; align-items: center; gap: 15px; color: white;">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                <div>
                    <h1 style="margin: 0; font-size: 1.5rem; font-weight: 800; letter-spacing: 0.5px; line-height: 1;">SICER <span style="font-weight:400; font-size:0.9rem;">IA</span></h1>
                    <span style="font-size: 0.75rem; font-weight: 600; opacity: 0.9; text-transform: uppercase; letter-spacing: 1px;">SISTEMA INTELIGENTE DE CERTIFICADOS</span>
                </div>
            </div>
            <div style="display: flex; align-items: center; gap: 15px; color: white;">
                <div style="text-align: right;">
                    <h2 style="margin: 0; font-size: 1.1rem; font-weight: 800; line-height: 1.2;">{st.session_state.usuario_actual.upper()}</h2>
                    <span style="font-size: 0.85rem; font-weight: 600; opacity: 0.9; color: #FCD34D;">{rol_text} ▾</span>
                </div>
                <div style="width: 45px; height: 45px; border-radius: 50%; background-color: rgba(255,255,255,0.15); border: 2px solid rgba(255,255,255,0.4); display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 1.2rem;">
    ...
    if st.session_state.rol == "administrador": menu = ["Inicio", "Usuarios", "Transacciones", "Configuración", "Motor IA"]
    elif st.session_state.rol == "emisor": menu = ["Inicio", "Transacciones"]
    else: menu = ["Solicitar Certificado", "Mis Operaciones"]

    # Inyectar CSS SVG Dinámico para el menú actual
    inject_sidebar_svg_css(menu)

    with st.sidebar:
        st.markdown("<br>", unsafe_allow_html=True) # Espaciador bajo el topbar
    ...
    elif seleccion == "Configuración": tab_configuracion_diseno()
